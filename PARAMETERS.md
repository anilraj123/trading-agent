...
the default expression. The nightly analyst requests the expression per thesis
(`instrument: shares|option`); an option request still has to clear every
deterministic gate — conviction ≥ 4, account options level, premium sleeve
room, and a contract passing every filter — or it falls back to shares
(bullish) / skips (bearish). Contract selection, sizing, and exits remain
fully deterministic.
...
| Parameter | Env Var | Default | Meaning |
|---|---|---|---|
| Per-thesis risk cap | `V2_MAX_RISK_PCT` | 0.05 | worst-case fill (zone high; zone low for bearish) to invalidation ≤ this; research rejects a breaching thesis, a loosening revision past it is refused (`revisions[].rejected`). Must be ≤ the disaster stop. Replaces the old 15% "invalidation depth" sanity bound |
| Lesson trade count | `V2_LESSON_MIN_TRADES` | 3 | lessons carry `(n=k)` distinct-trade counts; below this the research/post-mortem/weekly prompts treat them as tentative observations and forbid URGENT/HARD wording |
| Exit follow-up window | `V2_POSTMORTEM_FOLLOWUP_DAYS` | 10 | post-mortem also sees exits from the last N days with `since_exit_pct` (price now vs exit) so exit lessons rest on the post-exit path; weekly review gets 28 days |
| Options master switch | `V2_OPT_ENABLED` | **false in compose** (code default true) | 2026-09-02: options off until the stock side shows an edge; bearish theses are rejected at validation while off |

## trader_v2 Share Fill Confirmation (`trader_v2/config.py` + `executor._share_fill`)

| Parameter | Env Var | Default | Meaning |
|---|---|---|---|
| Share fill wait | `V2_SHARE_FILL_WAIT_SEC` | 30 | how long to poll a share order for the broker's own `filled_avg_price` before giving up and booking the quote. Market orders fill in well under a second in RTH — this budget only covers a stalled venue |
| Share fill poll | `V2_SHARE_FILL_POLL_SEC` | 1 | poll interval within that wait (options keep the 5 s cadence). Must be ≤ the wait |

A timeout books the pre-trade quote, journals `fill_estimated: true` on the
entry/exit event, and sends a high-priority notification — an unconfirmed price
is never presented as fact, because the disaster stop is anchored on it.

## trader_v2 Trail Floor (`trader_v2/config.py` + `thesis.trail_stop_level`)

| Parameter | Env Var | Default | Meaning |
|---|---|---|---|
| Trail floor at entry | `V2_TRAIL_FLOOR_AT_ENTRY` | true | once the trail is ARMED its exit level never sits below the entry: `max(hwm × (1 + V2_TRAIL_STOP_PCT), entry_price)`. `false` restores the raw give-back without a redeploy |
| Invalidation exit | `V2_INVALIDATION_EXIT_ENABLED` | **false in compose** (code default true) | 2026-09-12: drops the invalidation rung from `exit_decision`. The **disaster stop is unaffected** and remains the loss rail |
| Per-position cap | `V2_POSITION_CAP_PCT` | **0.10 in compose** (code default 0.30) | 2026-09-12 de-risk: at 0.10 this BINDS below the equal-weight `capital/MAX_POSITIONS` slice (~$96 vs ~$240 on $961), so the book runs ~40% invested instead of ~100% |

At the defaults the raw trail level at the arming point is
`entry × 1.03 × 0.97 = 0.9991 × entry` — **below** the entry, so arming the
trail used to guarantee a small loss. The floor binds only while
`hwm < entry / (1 + V2_TRAIL_STOP_PCT)` (≈ +3.09%); above that the raw level is
already higher. **It removes the losing-trail case, it does NOT fix winner
truncation** — the give-back is set by `V2_TRAIL_STOP_PCT`, so a peak of +3.67%
still exits at +0.56% (LEU). Narrowing the give-back or raising the arming bar
is the separate, unmade decision.
...
| Date | Change |
|---|---|
| Sep 12 | **Invalidation exit off + position size cut to 0.10 (user-approved on backtest evidence).** A point-in-time backtest harness (`research/`, 1,213 symbols incl. delisted, Wikipedia revision-derived monthly index membership) shows the deployed strategy returns **−77.2% over 2016-2026** and that neither the live screen nor a momentum family survives walk-forward: **1/4 folds positive vs SPY** for both, median alpha −20.2% and −26.4%. SPY buy-and-hold beat every one of ~200 configurations tested. (1) `V2_INVALIDATION_EXIT_ENABLED` false — invalidation-off was selected in **16/16** walk-forward fold-selections across both families and both universes; on the biased run the rung alone was 315 trades at a **0% win rate** for −$22,768, and the live analyst independently wrote the same lesson after SBAC's −2.39% invalidation exit round-tripped back to entry. `exit_decision` gains `invalidation_enabled`; the −8% disaster stop is deliberately untouched and tests assert it still fires. (2) `V2_POSITION_CAP_PCT` 0.30 → 0.10, which becomes the binding sizing constraint (~$96/position on $961 vs the ~$240 equal-weight slice) — de-risking while the strategy has no demonstrated edge rather than paying tuition for information already in hand. **Neither change makes the strategy profitable; both reduce the rate of loss.** 9 tests added, 334 pass. |
| Sep 9 | **Trail floor at entry — an armed trail no longer aims below the entry.** With `V2_TRAIL_ACTIVATE_PCT` +3% and `V2_TRAIL_STOP_PCT` −3%, the exit level at the arming point is `entry × 1.03 × 0.97 = 0.9991 × entry`, i.e. BELOW the entry — arming the trail guaranteed a small loss. Live proof: SMR armed at +3.0% and exited −0.14% (true fill basis). New `thesis.trail_stop_level(t, trail_pct, floor_at_entry)` returns `max(hwm × (1 + trail_pct), entry_price)`; `exit_decision` takes `trail_floor_at_entry`, wired to `V2_TRAIL_FLOOR_AT_ENTRY` (default true, `false` reverts without a redeploy). **Scope, stated plainly: this fixes the losing-trail case only.** The floor binds solely while `hwm < entry / 0.97` (≈ +3.09%); beyond that the raw level is already above entry, so LEU (peak +3.67%, exit +0.56%) is completely unaffected. Winner truncation is driven by `V2_TRAIL_STOP_PCT`'s give-back, not by this floor — narrowing the give-back or raising the arming bar remains an open, deliberately unmade decision. 7 tests added, including SMR and LEU as explicit regression cases. 325 pass (was 318). |
| Sep 8 | **Share fills were never read back from the broker — every share P&L, and every entry-anchored stop, was wrong.** `executor` read `order.filled_avg_price` off the response to `submit_market_order`, but a market order returns ACCEPTED with that field `None`, so it always fell through to `price`, the PRE-TRADE QUOTE; the exit path did not even try (`close_position(); fill = price`). The option path had polled correctly since Aug 10 — shares never did. Measured against Alpaca's own FILL activities, the 7 closed live trades realized **−$50.19, not the −$34.28 the journal claimed** (46% understated), and the true record is 1 winner in 7, not 2. Worst case NBTX: filled 44.7912, booked 43.75, so its −8% disaster stop sat at 40.25 instead of 41.21 — the 9/1 quote of 40.625 passed under the wrong threshold and the trade ran to −12.9% before a `research_close`, a $15.90 single-trade error. The corrupted P&L also fed every lesson in `lessons.md` and every ntfy alert. New `executor._share_fill` polls for the broker's average and qty on **both** share paths (`V2_SHARE_FILL_WAIT_SEC` 30, `V2_SHARE_FILL_POLL_SEC` 1); entries adopt the filled qty and journal `quote_price` + `slippage_pct`, exits journal `closed_qty`, and either path that cannot confirm books the quote with `fill_estimated: true` plus a high-priority push. A partial share exit now warns that the stub is untracked. Also fixed a dead guard in `_wait_for_fill`: `str(OrderStatus.PARTIALLY_FILLED)` starts with `"OrderStatus."`, so the old `endswith("filled") and not startswith("partial")` pair accepted a PARTIAL fill as complete — now compares the bare member name via `_status_name`. No tunable thresholds changed. 16 tests added (new `tests/test_v2_executor.py`; the lessons-bounding test now pins its bound on V2Config directly, since the env pin only won while it was the first module to import config). |
...
| Aug 21 | **Twice-daily analyst automation + gated auto-deploy.** Two cloud routines (claude.ai/code/routines) analyze the bot every weekday — pre-market (~12:17 UTC) and post-close (~21:47 UTC, after nightly research + backup) — reading state from the Google Drive backup (they hold no Alpaca keys, so they can never trade) and opening/merging PRs for algo improvements. New `auto-deploy-main.sh` on the droplet (cron weekdays 13:10 UTC) ships merged main pre-market, gated on the unit tests passing inside the freshly built image (pytest added to requirements.txt); build/test failures block the deploy and page via ntfy. Backup cron moved 21:00→21:30 UTC so it always captures same-day nightly research across DST (EST close is 21:00 UTC — the old time raced it). |
| Aug 21 | **LIVE CUTOVER — trader_v2 takes the full real-money account** (user-directed; see `CUTOVER.md`). Compose reduced to a single `trader-v2` service: live endpoint explicit, `V2_REQUIRE_PAPER=false`, `V2_CAPITAL_MODE=dynamic` (sizing + options sleeve anchor to live equity), `DATA_DIR=/app/data/v2live` (fresh book/journal/baseline; paper lessons.md copied over). v1 equity bot service + `options_bot/` retired (`trader/` remains as a shared library). v1's 7 legacy positions flattened (fractional market orders queue for the open). Droplet `.env` shakeout override `V2_OPT_MIN_CONVICTION=3` reverted → default 4. Startup notif no longer hardcodes "(paper)"; daily summary reports against the baseline equity in dynamic mode (static keeps `V2_CAPITAL`). Backup script now execs rclone in `trader-v2` (old script targeted the deleted `trading-agent` container); it copies all of `/app/data` so `v2live/` is covered. |
| Aug 21 | **v2 thesis-driven instrument choice.** Root cause of the stalled options shakeout: `choose_instrument` hard-required a `catalyst_date`, but the analyst's inputs (TA screens + headlines) almost never surface verifiable dated events and it was told never to fabricate one — 11 days, zero option entries, every entry `fallback_reason:no_catalyst_date`. Fix: the analyst now requests the expression per thesis via a new `instrument` field (`shares`\|`option`, default shares) in the research schema, stored as `requested_instrument`; bearish is always an option request (puts are its only expression). `choose_instrument` gates the REQUEST (not_requested → enabled → level ≥ 2 → conviction ≥ `V2_OPT_MIN_CONVICTION` → sleeve) — the dated-catalyst gates are gone; `catalyst_date` is now supporting evidence, still schema-validated when present. Bearish validation drops the dated-catalyst requirement (conviction bar stays); bearish+`instrument:"shares"` is rejected. Prompt rewritten: option = decisive move expected within TTL, theta warning explicit. Legacy theses migrate to `requested_instrument:"shares"` (bearish → `"option"`). 8 tests net (245 pass). |
